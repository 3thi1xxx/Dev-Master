...
import { paperTradingSimulator } from './PaperTradingSimulator.js';
import { whaleDataService } from './WhaleDataService.js';
import { degenPaperTrader } from './DegenPaperTrader.js';
import { fastMemeAnalyzer } from './FastMemeAnalyzer.js';
import { momentumTracker } from './MomentumTracker.js';
import { birdeyeWebSocketManager } from './BirdeyeWebSocketManager.js';
...
export class LiveTokenAnalyzer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.config = {
      ...
      minLiquidity: options.minLiquidity || 500, // $500 minimum - catch more early gems
    ...
    if (data.room) {
      switch (data.room) {
        ...
        case 'sol-priority-fee':
          // Store fee data for optimal transaction timing
          this.feeData = this.feeData || {};
          this.feeData[data.room] = {
            fee: data.content,
            timestamp: Date.now()
          };
          break;
        
        // 🚀 CRITICAL: Handle surge-updates room (NEW TOKEN INTELLIGENCE)
        case 'surge-updates':
          this.processSurgeUpdates(data.content);
          break;
  ...
  /**
   * 🚀 CRITICAL: Process surge-updates for new token intelligence
   */
  async processSurgeUpdates(content) {
    try {
      // Safe parsing - avoid forEach errors
      if (!content || !content.updates || !Array.isArray(content.updates)) {
        console.warn('[SURGE] Invalid surge content structure');
        return;
      }

      console.log(`[SURGE] 📈 Processing ${content.updates.length} surge updates`);

      // Process each surge update safely
      for (const update of content.updates) {
        await this.processSingleSurgeUpdate(update);
      }

    } catch (error) {
      console.error('[SURGE] ❌ Error processing surge updates:', error.message);
    }
  }

  /**
   * Process individual surge update with comprehensive analysis
   */
  async processSingleSurgeUpdate(update) {
    try {
      const surgeData = update.surgeData;
      if (!surgeData || !surgeData.tokenAddress) {
        return;
      }

      const tokenAddress = surgeData.tokenAddress;
      const tokenInfo = {
        address: tokenAddress,
        name: surgeData.tokenName,
        ticker: surgeData.tokenTicker,
        
        // Price & Market Data
        price: surgeData.priceSol,
        marketCap: surgeData.marketCapSol,
        volume: surgeData.volumeSol,
        liquidity: surgeData.liquiditySol,
        
        // Trading Activity
        buyCount: surgeData.buyCount || 0,
        sellCount: surgeData.sellCount || 0,
        transactionCount: surgeData.transactionCount || 0,
        numHolders: surgeData.numHolders || 0,
        
        // Risk Metrics
        top10HoldersPercent: surgeData.top10HoldersPercent || 0,
        devHoldsPercent: surgeData.devHoldsPercent || 0,
        snipersHoldPercent: surgeData.snipersHoldPercent || 0,
        bundlersHoldPercent: surgeData.bundlersHoldPercent || 0,
        
        // Momentum Indicators
        currentRank: surgeData.currentRank,
        previousRank: surgeData.previousRank,
        rankJump: surgeData.rankJump,
        timeTakenSeconds: surgeData.timeTakenSeconds,
        
        // Social Intelligence
        twitter: surgeData.twitter,
        website: surgeData.website,
        telegram: surgeData.telegram,
        
        // Protocol Info
        protocol: surgeData.protocol,
        dexPaid: surgeData.dexPaid,
        
        // Timestamps
        createdAt: surgeData.createdAt,
        detectedAt: surgeData.detectedAt,
        openTrading: surgeData.openTrading
      };

      // Store in cache for momentum tracking
      this.updateTokenCache(tokenAddress, tokenInfo);

      // Apply filters for analysis triggers
      if (this.shouldAnalyzeToken(tokenInfo)) {
        console.log(`[SURGE] 🎯 Analyzing promising token: ${tokenInfo.ticker} (${tokenInfo.name})`);
        
        // Emit for external analysis
        this.emit('new_token_surge', tokenInfo);
        
        // Trigger internal analysis if auto-analyze is enabled
        if (this.config.autoAnalyze) {
          this.queueTokenAnalysis(tokenAddress, tokenInfo.ticker);
        }
      }

    } catch (error) {
      console.error('[SURGE] ❌ Error processing single surge update:', error.message);
    }
  }

  /**
   * Advanced filtering logic for token analysis
   */
  shouldAnalyzeToken(tokenInfo) {
    // Basic validation
    if (!tokenInfo.price || !tokenInfo.liquidity || !tokenInfo.volume) {
      return false;
    }
    ...
    // Risk filters
    if (tokenInfo.devHoldsPercent > 10) { // Dev holds > 10%
      console.log(`[SURGE] 🚨 Skipping ${tokenInfo.ticker}: High dev holdings (${tokenInfo.devHoldsPercent}%)`);
      return false;
    }

    if (tokenInfo.top10HoldersPercent > 80) { // Too concentrated
      console.log(`[SURGE] 🚨 Skipping ${tokenInfo.ticker}: High concentration (${tokenInfo.top10HoldersPercent}%)`);
      return false;
    }

    // Momentum filters
    if (tokenInfo.rankJump && tokenInfo.rankJump > 0) { // Positive momentum
      return true;
    }

    if (tokenInfo.buyCount > tokenInfo.sellCount * 2) { // Strong buying pressure
      return true;
    }

    if (tokenInfo.volume > tokenInfo.liquidity * 0.1) { // Good volume/liquidity ratio
      return true;
    }
  ...
  /**
   * Update token cache for momentum tracking
   */
  updateTokenCache(tokenAddress, tokenInfo) {
    if (!this.tokenCache) this.tokenCache = new Map();
    
    const existing = this.tokenCache.get(tokenAddress);
    if (existing) {
      // Track changes
      tokenInfo.priceChange = existing.price ? (tokenInfo.price - existing.price) / existing.price : 0;
      tokenInfo.volumeChange = existing.volume ? tokenInfo.volume - existing.volume : 0;
      tokenInfo.holderChange = existing.numHolders ? tokenInfo.numHolders - existing.numHolders : 0;
    }
